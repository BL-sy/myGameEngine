...
namespace Hazel {
	...
	Ref<IndexBuffer> Hazel::IndexBuffer::Create(uint32_t* indices, uint32_t size)
	{
		// 决定使用哪个API，实例化的类型
		switch (Renderer::GetAPI())
		{
		case RendererAPI::APIType::None:	HZ_CORE_ASSERT(false, "None is now not suported!");
		case RendererAPI::APIType::OpenGL:	return CreateRef<OpenGLIndexBuffer>(indices, size); // 这里size是count
		}
